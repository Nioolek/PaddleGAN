...
class NAFNetDFL(nn.Layer):
    ...
    def __init__(self,
        ...
        self.s0_n = s0_n
        self.proj_conv = nn.Conv2D(self.reg_max, 1, 1, bias_attr=False)
        self.proj_conv.skip_quant = True
        # 权重全部减去self.s0_n
        proj = (paddle.linspace(0, self.reg_max - 1, self.reg_max).reshape(
            [1, self.reg_max, 1, 1]) - self.s0_n) * 20. / 255.
        ...
        self.ending = nn.Conv2D(in_channels=width,
                                out_channels=img_channel*self.reg_max,
                                kernel_size=3,
                                padding=1,
                                stride=1,
                                groups=1,
    ...
    def forward(self, inp):
        ...
        # (bs, 3*reg_max, h, w) -> (bs, h*w*3, reg_max)
        l = H*W


        # reg_dist = x.reshape([B, 3, self.regmax, l]).transpose([0, 2, 3, 1])
        # reg_dist = self.proj_conv(F.softmax(reg_dist, axis=1)).squeeze(1)
        # pred_dist = F.softmax(pred_dist.reshape([-1, l, 4, self.reg_max]))
        # (B, reg_max * 3, H, W) -> (B, H * W, reg_max * 3)
        reg_dist = x.flatten(2).transpose([0, 2, 1])
        pred_dist = F.softmax(reg_dist.reshape([-1, l, 3, self.reg_max]))
        x = self.proj_conv(pred_dist.transpose([0, 3, 1, 2])).squeeze(1)
        # x (bs, H*W, 3) -> (bs, 3, H, W)
        x = x.reshape([B, H, W, 3]).transpose([0, 3, 1, 2])
        # print('111xxx', x.shape)
